...
const arr5 = person.skills.map((skill) => {
  return (
    skill.at(0).toUpperCase() + skill.slice(1, -1) + skill.at(-1).toUpperCase()
  );
});
console.log(arr5);
...
const arr7 = person.friends.map(({ name, surname, age }) => {
  return `${name} ${surname} are ${age} ani.`;
});
console.log(arr7);
...
const arr9 = person.friends.map(({ name, surname, age }) => {
  return Math.abs(name.length + surname.length - age);
});
console.log(arr9);
